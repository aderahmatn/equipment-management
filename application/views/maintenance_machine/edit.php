	...
	<div class="col-md-12">
		<div class="card  box-shadow-0">
			<div class="card-header">
				<h4 class="card-title mb-1">Edit Transaction Maintenance Machine Data</h4>
				<p class="mb-2">Please fill the form for update data</p>
			</div>
			<div class="card-body">
				<form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
					<input type="hidden" name="id_transaction_maintenance_machine" value="<?= $data->id_transaction_maintenance_machine ?>">
					<div class="row">
						<div class="col-md-6">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select id='id_master_occurence' class="form-control <?= form_error('id_master_occurence') ? 'is-invalid' : '' ?>" name="id_master_occurence">
									...
									<option value="0" <?= $data->occurence_value == 0 ? 'selected' : '' ?>>None</option>
									<?php foreach ($occurence as $key) : ?>
										<option value="<?= $key->occurence_value ?>" <?= $data->occurence_value == $key->occurence_value ? 'selected' : '' ?>><?= strtoupper($key->occurence_type) ?></option>
									<?php endforeach ?>
								</select>
								<div class="invalid-feedback">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select id='id_master_severity' class="form-control <?= form_error('id_master_severity') ? 'is-invalid' : '' ?>" name="id_master_severity">
									...
									<option value="0" <?= $data->severity_value == 0 ? 'selected' : '' ?>>None</option>
									<?php foreach ($severity as $key) : ?>
										<option value="<?= $key->severity_value ?>" <?= $data->severity_value == $key->severity_value ? 'selected' : '' ?>><?= strtoupper($key->severity_type) ?></option>
									<?php endforeach ?>
								</select>
								<div class="invalid-feedback">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select id='id_master_detection' class="form-control <?= form_error('id_master_detection') ? 'is-invalid' : '' ?>" name="id_master_detection">
									...
									<option value="0" <?= $data->detection_value == 0 ? 'selected' : '' ?>>None</option>
									<?php foreach ($detection as $key) : ?>
										<option value="<?= $key->detection_value ?>" <?= $data->detection_value == $key->detection_value ? 'selected' : '' ?>><?= strtoupper($key->detection_type) ?></option>
									<?php endforeach ?>
								</select>
								<div class="invalid-feedback">
					...
					<div class="row">
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<input type="date" id='date_maintenance_machine' class="form-control <?= form_error('date_maintenance_machine') ? 'is-invalid' : '' ?>" name="date_maintenance_machine" value="<?= $data->date_maintenance_machine ?>">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" id='frpn_value' class="form-control <?= form_error('frpn_value') ? 'is-invalid' : '' ?>" name="frpn_value" readonly value="<?= $data->frpn_value ?>">
...
<script>
	$(document).ready(function() {
		var idequip = '<?= $data->id_master_equipment ?>'
		$.ajax({
