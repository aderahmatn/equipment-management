...
class Maintenance_machine extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($validation->run() == FALSE) {
            ...
            $this->template->load('layouts/index', 'maintenance_machine/index', $data);
        } else {
            $post = $this->input->post(null, TRUE);
            $maintenance_machine->Add($post);
    ...
    public function get_overall_frpn()
    {
        $id = $this->input->post('id');
        $data = $this->Maintenance_machine_model->get_frpn_by_equipment($id);
        echo json_encode($data);
    }
